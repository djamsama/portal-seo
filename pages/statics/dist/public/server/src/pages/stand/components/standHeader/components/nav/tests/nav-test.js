...
test("test stand header structure", function () {
  var standHeaderNav = mountWithIntl(/*#__PURE__*/_react["default"].createElement(_context.AppContext.Provider, {
    value: context
  ...
  /* Le Composant est composé d'un styled component 'HeaderStandNavContainer'. */
  expect(standHeaderNav.find(_nav.HeaderStandNavContainer)).toHaveLength(1);

  /* Qui lui même est composé d'un composant 'MobileNavDropdown'. */
  expect(standHeaderNav.find(_nav.HeaderStandNavContainer).find(_nav.MobileNavDropdown)).toHaveLength(1);
});