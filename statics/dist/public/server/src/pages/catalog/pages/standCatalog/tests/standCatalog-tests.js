"use strict";

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");
var _react = _interopRequireDefault(require("react"));
var _reactRedux = require("react-redux");
var _reduxMockStore = _interopRequireDefault(require("redux-mock-store"));
var _reduxThunk = require("redux-thunk");
var _styledComponents = require("styled-components");
var _mockContext = require("../../../../../../tools/utils-test/mockContext");
var _mockMenu = require("../../../../../../tools/utils-test/mockMenu");
var _theme = require("../../../../../components/theme");
var _catalog = _interopRequireDefault(require("../../../container/catalog"));
var _pageLayout = _interopRequireDefault(require("../../../../../layouts/pageLayout"));
var _excerpt = _interopRequireDefault(require("../components/excerpt"));
var _buttonsContainer = _interopRequireDefault(require("../components/buttons/containers/buttonsContainer"));
var _mainCatalog = _interopRequireDefault(require("../components/mainCatalog"));
var _context = require("../../../../../context");
var _index = _interopRequireDefault(require("../index"));
var _mockAdapter = require("../../../../../../tools/utils-test/mockAdapter.js");
require("../../../../../../tools/utils-test/mockIntersectionObserver.js");
(0, _mockAdapter.doSomeMock)();
(0, _mockAdapter.doMockCatalogButtons)();
var t = _theme.theme.ae;
var mockStore = (0, _reduxMockStore["default"])([_reduxThunk.thunk]);
var state = {
  Menu: _mockMenu.menu,
  Application: {
    currentCurrency: "fr",
    localeProp: "fr",
    extendedLanguage: false
  }
};
var store = mockStore(state);
var showRelatedLinks = true;
test("StandCatalog", function () {
  /* eslint-disable no-undef, no-unused-vars */ // $FlowFixMe
  var StandCatalog = mountWithIntl(/*#__PURE__*/_react["default"].createElement(_context.AppContext.Provider, {
    value: _mockContext.context
  }, /*#__PURE__*/_react["default"].createElement(_reactRedux.Provider, {
    store: store
  }, /*#__PURE__*/_react["default"].createElement(_styledComponents.ThemeProvider, {
    theme: t
  }, /*#__PURE__*/_react["default"].createElement(_index["default"], {
    showRelatedLinks: showRelatedLinks
  })))));
  expect(StandCatalog.find(_pageLayout["default"])).toHaveLength(1);
  expect(StandCatalog.find(_catalog["default"])).toHaveLength(1);
  expect(StandCatalog.find(_excerpt["default"])).toHaveLength(1);
  expect(StandCatalog.find(_mainCatalog["default"])).toHaveLength(1);
  expect(StandCatalog.find(_buttonsContainer["default"])).toHaveLength(1);
});